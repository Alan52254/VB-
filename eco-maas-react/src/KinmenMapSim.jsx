...
const KinmenMapSim = ({ onSimulationUpdate, isRunningExternal }) => {
  ...
  const [metrics, setMetrics] = useState({
    totalEnergy: 0, totalServed: 0, totalDist: 0, platoonDist: 0, emptyDist: 0, totalWaitTime: 0
  });

  // 🔥 1. 使用 Ref 來解決 setInterval 閉包問題
  const latestDataRef = useRef({ vehicles: [], gameTime: 0, metrics: {} });

  // 🔥 2. 隨時同步最新數據到 Ref
  useEffect(() => {
    latestDataRef.current = { vehicles, gameTime, metrics };
  }, [vehicles, gameTime, metrics]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const mappedVehicles = vehicles.map(v => ({
        id: `Bus-${v.id}`,
        zone: "金門區域", // 這裡可以加入更詳細的區域判斷邏輯
        soc: v.battery,
        status: v.status === 'moving' ? 'Service' : 'Charging',
  ...
  const resetSimulation = () => {
    const newVehicles = Array.from({ length: 6 }).map((_, i) => ({
      ...
      power: 0, platooning: false, totalDist: 0, aiState: 'INIT',
      boardedLast: 0, alightedLast: 0
    }));
    setVehicles(newVehicles);

    const newStations = LOCATIONS.map(loc => ({ ...loc, queue: 0 }));
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [isRunning, vehicles, gameTime, mode]); // 這裡依賴 vehicles 是為了觸發下一次計算，但在 React 中這種寫法雖然不是效能最好但邏輯可行
      ...
      return prevVehicles.map(v => {
        ...
        if (mode === 'rl') {
            isPlatooning = prevVehicles.some(other => 
                other.id !== v.id && calcDist(v, other) < PLATOON_DISTANCE && calcDist(v, other) > 10 
            );
        }
        ...
        return {
          ...
          passengers, aiState: decision.action, boardedLast: currentBoarded, alightedLast: currentAlighted
        };
      });
    });
